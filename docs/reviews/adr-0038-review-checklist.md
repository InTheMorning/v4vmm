...
| 4 | HIG Dark-Mode Parity Audit                    | In progress    | `src/ui/style.rs` no longer holds raw `gpui::rgb(0x…)` literals — the four ID3 frame-color helpers (`id3_frame_v22`, `id3_frame_v23_only`, `id3_frame_v24_only`, `id3_frame_unknown`) now resolve through new `SemanticColor::Id3FrameV22 / V23Only / V24Only / Unknown` tokens with light/dark and high-contrast variants. Module-level docs in `src/ui/style.rs` updated to declare the file token-resolved. New architecture test `ui_style_resolves_colors_through_token_layer` blocks any future raw rgb literal in `src/ui/style.rs`. Remaining work: capture light + dark screenshot pairs per surface in the Visual Smoke Ledger (currently blocked — environment-dependent). |
| 5 | HIG Accessibility-Label Contract              | In progress    | Slices: `ActionButtonDisplay` carries `a11y_label` sourced via `EntityActionVm::a11y_label()` / `LibraryAlbumMusicBrainzActionVm::a11y_label`; `identity_action_button` migrated to `IdentityActionButtonDisplay` sourced via `IdentityActionDisplay::a11y_label` / `ContributorIdentityActionDisplay::a11y_label` (per-kind defaults from `IdentityActionDisplayKind::default_a11y_label` / `ContributorIdentityActionKind::default_a11y_label`); `interactive_composites_carry_accessibility_labels` guard scans both. GPUI 0.2.x lacks an accessibility-label sink on its button widget; the field is contract-only today. Remaining composites follow in subsequent slices. |
...
|---|---|---|---|---|
| `action_button`              | `EntityActionVm::a11y_label()` / `LibraryAlbumMusicBrainzActionVm::a11y_label` | No (label suffices for current call sites) | `interactive_composites_carry_accessibility_labels` (`ActionButtonDisplay`) | Migrated 2026-05-04 |
| `ActionRow`                  | TBD | TBD | TBD | Pending |
| `identity_action_button`     | `IdentityActionDisplay::a11y_label` / `ContributorIdentityActionDisplay::a11y_label` (default per kind) | No (kind-default phrasing already disambiguates) | `interactive_composites_carry_accessibility_labels` (`IdentityActionButtonDisplay`) | Migrated 2026-05-04 |
