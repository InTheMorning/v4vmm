...
| 6 | PageVm Generalization                         | In progress    | Track detail parity slice landed: `TrackDetailPageVm` wraps `TrackDetailVm`, `src/ui/shells/track.rs::build_track_detail_surface` is the shared helper, Library/Discover track detail no longer construct `TrackDetailSurface` directly, and `entity_detail_pages_render_through_shell_helper_and_page_vm` guards the explicit release + track surface list. Remaining: artist and playlist PageVms, plus a decision on search results / recent-feed tile batch VM scope. |
