...
Amended 2026-09-18: all three ADR 0039 packets are complete.
Their scheduled position follows recovery task 013 and precedes Settings
follow-through and relay adoption. Task 001 introduced separate resolvers
without changing values. Task 002 added the reservation and ShowCard correction.
Task 003 introduced the ratified type curves and owned all thirteen visual checks.

Mechanical checks are Green. The configuration format is unchanged.

Status reconciled 2026-09-18: ADR 0069 phase 002 can use the completed recovery
owners. Its packet remains unwritten. Phase 003 still requires full ADR 0066
acceptance. Task 004 retains that gate while playback work remains deferred.
The [status review](../reviews/2026-09-18-adr-status-and-remaining-work.md)
records corrected packet labels and the remaining decision work.
...
Completed 2026-09-18: the operator passed all thirteen ADR 0039 visual checks
and confirmed fixture removal. The agent inferred preservation from the
conditional cleanup command. The evidence is recorded in the
[type review](../reviews/adr-0039-review-checklist.md#operator-visual-check--task-003).
ADR 0039 is Implemented. Its scheduled work is complete.
...
| 2, Settings follow-through | [ADR 0069 editor, metadata and preset phases](adr-0069-settings-presets-phase-plan.md#sequence-and-stopping-points) | Phase 002 prerequisites are met. Author its bounded packet next. New persisted mode/resource keys and presets require full ADR 0066 acceptance. Audio remains deferred. |
...
| `musicindex-live-publisher` | show log 002 | Not started. Follows 001. Corrected reader instructions use observed_at for episode and operator ranges. |
