...
use crate::application::commands::download::{
    RemoveCachedFiles, SubscribeThenAppendToPlaylist, SubscribeTrack,
};
...
use crate::library::{build_tree, playlist_options, LibraryApp, LibraryAppEvent};
...
#[derive(Clone)]
enum RemoteDetailThumbnailState {
    Loading,
...
impl TopApp {
    ...
    fn new(
        ...
        Self {
            ...
            content_pane_width: layout::CONTENT_PANE_DEFAULT_WIDTH,
    ...
    fn open_search_results_in_content_list(&mut self, query: &str, cx: &mut Context<Self>) {
    ...
    fn start_index_search_for_query(&mut self, query: &str, cx: &mut Context<Self>) {
        ...
        match tab {
            ...
            AppTab::Library => {
                ...
                let entry = self
                    .workspace_layout
                    .frame_nav(content_list_id)
                    .map_or(FrameNavigationEntry::SourceList, |nav| {
                        nav.current().clone()
                    });
    ...
    fn content_list_frame_id(&self) -> Option<WorkspaceFrameId> {
    ...
    fn content_list_breadcrumb_labels(
        &self,
    ...
    fn sync_search_results_detail_with_nav(&mut self, content_list_id: WorkspaceFrameId) {
    ...
    fn handle_search_result_selected(
        &mut self,
        tab: SearchResultsTab,
        result_id: &str,
        cx: &mut Context<Self>,
    ) {
        // Get the ContentList frame id
        ...
        match tab {
            SearchResultsTab::Tracks => {
                ...
                if let Ok(track_id) = track_id_str.parse::<i64>() {
                    if let Some(track_row) = self.conn.lock().ok().and_then(|conn| {
                        ...
                        // Select the track in LibraryApp
                        self.library.update(cx, |library, cx| {
                            library.select_track(&track_row, cx);
                        });
                        // Push TrackDetail navigation entry to ContentList frame
            ...
            SearchResultsTab::Feeds => {
                ...
                if let Ok(feed_id) = result_id.parse::<i64>() {
                    // Detail navigation must include cached Index rows that are no longer
                    // present in the source tree after removal.
                    let album_found = self.library.read(cx).album_for_detail_by_feed_id(feed_id);
                    if let Some(album) = album_found {
                        self.library.update(cx, |library, cx| {
                            library.select_album(&album, cx);
                        });
                        // Push AlbumDetail (or FeedDetail if that's the pattern) navigation entry
                        if let Err(e) = self
                            .workspace_layout
            ...
            SearchResultsTab::Artists => {
                ...
                let Some(artist_name) = result_id.strip_prefix("library-artist:") else {
                    // Log and skip if prefix is absent (future Index-origin row case)
                ...
                // Push ArtistDetail navigation entry
                if let Err(e) = self.workspace_layout.push_nav(
                    content_frame_id,
        ...
        // Keep the active search VM synchronized with the ContentList nav flow.
        self.sync_search_results_detail_with_nav(content_frame_id);
    }
    ...
    fn handle_content_list_breadcrumb_select(
        &mut self,
        entry: FrameNavigationEntry,
        cx: &mut Context<Self>,
    ) {
    ...
    fn set_content_pane_width(&mut self, width: gpui::Pixels, cx: &mut Context<Self>) {
        self.content_pane_width = width;
        cx.notify();
    }

    fn begin_content_pane_resize(&mut self, cx: &mut Context<Self>) {
        self.is_content_pane_resizing = true;
        cx.notify();
    }

    fn resize_content_pane(&mut self, x: f32, cx: &mut Context<Self>) {
    ...
    fn end_content_pane_resize(&mut self, cx: &mut Context<Self>) {
            ...
            .filter_map(|frame| match frame.kind() {
                WorkspaceFrameKind::ContentList => {
                    if content_seen {
                        ...
                        content_seen = true;
                        let title = Self::content_list_frame_title(layout, frame.id(), mount);
    ...
    fn render_workspace_content(
        ...
        // Stage 2: Body switch is exhaustive on nav top
        let current_nav = self
            .workspace_layout
        ...
        let mut workspace_slots = match &current_nav {
                ...
                let inspector_slots = SearchResultsInspectorSlots::new()
                    ...
                    .on_clear_filter(move |_window, cx| {
                        clear_entity.update(cx, |this, cx| {
                            this.set_search_results_filter(
                                content_frame_id,
                                ContentFilter::All,
                                cx,
                            );
                ...
                WorkspaceSlots::new()
                    ...
                    .on_content_list_filter_select(move |filter, _window, cx| {
                        filter_entity.update(cx, |this, cx| {
                            this.set_search_results_filter(content_frame_id, filter, cx);
                        });
                    })
            }
                ...
                if self.search_results_detail.is_some() =>
            {
                ...
                let detail = self
                    ...
                    .as_ref()
                    .unwrap()
                    .index_feed_detail(&activation_id, id, label);
        ...
        let breadcrumb_labels = self.content_list_breadcrumb_labels(content_frame_id, cx);
        workspace_slots = workspace_slots.content_list_breadcrumb_labeler(move |entry| {
            breadcrumb_labels
                .iter()
                .find_map(|(candidate, label)| (candidate == entry).then(|| label.clone()))
                .unwrap_or_else(|| entry.display_label())
        });

        // Wire breadcrumb select handler for ContentList
        let breadcrumb_entity = entity.clone();
        workspace_slots =
            workspace_slots.on_content_list_breadcrumb_select(move |entry, _window, cx| {
                breadcrumb_entity.update(cx, |this, cx| {
                    this.handle_content_list_breadcrumb_select(entry, cx);
                });
            });

        let back_entity = entity.clone();
        workspace_slots = workspace_slots.on_content_list_back_select(move |_window, cx| {
            back_entity.update(cx, |this, cx| {
                this.handle_content_list_back_select(cx);
            });
        });
        ...
        workspace_slots = workspace_slots
            ...
            .on_content_pane_resize_start({
                let resize_entity = entity.clone();
                move |_window, cx| {
                    resize_entity.update(cx, |this, cx| {
                        this.begin_content_pane_resize(cx);
                    });
                }
            })
            .on_content_pane_resize_move({
                let resize_entity = entity.clone();
                move |x, _window, cx| {
                    resize_entity.update(cx, |this, cx| {
                        this.resize_content_pane(x, cx);
                    });
                }
            })
            .on_content_pane_resize_end({
                let resize_entity = entity.clone();
                move |_width, _window, cx| {
                    resize_entity.update(cx, |this, cx| {
                        this.end_content_pane_resize(cx);
                    });
                }
            });

        // Stage 5: Detail frame is not rendered in visible layout
